...
from services.clips.job_runner import (
    run_pipeline, run_finals_pipeline,
    register_task, get_task, cancel_task, recover_orphans,
)
...
async def test_run_pipeline_happy_path(tmp_path):
    ...
    for stage in expected:
        try:
            ...
            pytest.fail(f"Expected stage {stage!r} not found in order. Got: {progress_stages}")
...
async def test_run_finals_pipeline_publishes_stages_in_order(tmp_path):
    ...
    sb.table.return_value.select.return_value.in_.return_value.execute = AsyncMock(
        return_value=MagicMock(data=[
            {"id": "cand-1", "start_seconds": 0, "end_seconds": 30,
             "hype_score": 9, "hype_reasoning": "r", "transcript_excerpt": "e"},
            {"id": "cand-2", "start_seconds": 30, "end_seconds": 60,
             "hype_score": 8, "hype_reasoning": "r", "transcript_excerpt": "e"},
        ])
    )
    sb.table.return_value.update.return_value.eq.return_value.execute = AsyncMock()
    ...
    with patch("services.clips.job_runner.get_async_client", new=AsyncMock(return_value=sb)), \
         ...
         patch("services.clips.job_runner.fetch_transcript", new=AsyncMock(return_value=cues)), \
         patch("services.clips.job_runner.signed_url_helper",
               new=AsyncMock(return_value="https://signed.example/abc")), \
         patch("services.clips.job_runner.render_one_final", new=AsyncMock(side_effect=_render_one_final)), \
         patch("services.clips.job_runner.asyncio.create_subprocess_exec",
               new=AsyncMock(return_value=proc_mock)), \
         patch("services.clips.job_runner.broker", new=br_mock):
        await run_finals_pipeline(
            job_id=job_id, user_id=user_id,
            candidate_ids=candidate_ids, tmp_dir=tmp_path,
    ...
    expected = ["download_source", "extract_audio", "transcribe", "render_finals", "done"]
    seen_idx = -1
    for stage in expected:
        try:
            seen_idx = progress_stages.index(stage, seen_idx + 1)
    ...
    rp_by_cand = {cid: [e["pct"] for e in rp if e["candidate_id"] == cid]
