...
async def test_fetch_metadata_parses_ytdlp_json():
    ...
    with patch(
        "services.clips.metadata._run_ytdlp_dump", new=AsyncMock(return_value=fake_json)
    ):
...
async def test_fetch_metadata_invalid_url_propagates():
    with patch(
        "services.clips.metadata._run_ytdlp_dump",
        new=AsyncMock(side_effect=RuntimeError("yt-dlp failed")),
    ):
